...
async function main() {
  ...
  const publicUrl = 'https://elinext-lab.github.io/prototypes/mpet_2026-04-17_16-19-02/';
    ...
    const metrics = await page.evaluate((expectedUrl) => {
      ...
      for (let i = 1; i < blocks.length; i += 1) {
        ...
        if (curr.top < prev.bottom - 1) {
          overlaps.push({ previous: prev.name, current: curr.name, delta: Number((prev.bottom - curr.top).toFixed(2)) });
        }
      }
